...
def mi_first_forward(mels, lf0, encoder, encoder_lf0, encoder_spk, cs_mi_net, optimizer_cs_mi_net,
    ...
    np.random.seed(137)
    # Randomly mask mels by 100%
    mask_length = int(mels.shape[1]*1)  # Length of the mask
    mask_start = random.randint(0, mels.shape[1] - mask_length)  # Random starting point for the mask
    mels_masked = mels.clone()
    mels_masked[:, mask_start:mask_start + mask_length, :] = 0.0
 
    z, _, _, _, _ = encoder(mels)  # Feed masked mel spectrogram to the encoder
    z = z.detach()
    lf0_embs = encoder_lf0(lf0).detach()

    spk_embs = encoder_spk(mels_masked).detach()  # Feed masked mel spectrogram to the speaker encoder
...
def mi_second_forward(mels, lf0, encoder, encoder_lf0, cpc, encoder_spk, cs_mi_net, ps_mi_net, cp_mi_net, decoder, cfg, optimizer, scheduler):
    ...
    spk_embs = encoder_spk(mels_masked)
